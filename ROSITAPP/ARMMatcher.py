...
class LDRMatcherARM1(ARMMatcher):
    ...
    def _get_fix(self, groups, ptvalmarkers):
        ...
        if ptvalmarkers.find('L') >= 0 or ptvalmarkers.find('S') >= 0:
            ...
            inst_list = ["rors {}, {}".format(rt, rt), 
                     "{} {}, [{}]".format(inst, rd, r1X(r1))]

            prio = 30
        else:
            if rd not in set([r1]) and ptvalmarkers.find('Op1') >= 0:
...
class PrevANYMatcherARM(ARMMatcher):
    def __init__(self):
    ...
    def _get_fix(self, groups, ptvalmarkers):
        ...
        inst_list = []
        prio = 1
        if ptvalmarkers.find('M') >= 0:
            inst_list = ["push {{{}}}".format(rt),
                        "pop {{{}}}".format(rt)]
            prio = 20
        elif len(ptvalmarkers.intersection(set(['Op1*Op2','Op2*Op1']))) > 0:
            inst_list = ["movs {}, {}".format(rt,rt)]
            prio = 20
        elif len(ptvalmarkers.intersection(set(['B1','B2','WP1','WP2','DP1','DP2','B1I','B2I']))) > 0:
            inst_list = ["movs {}, {}".format(rt,rt)]
        inst_list += ["{}".format(groups[0][0])]
        return (inst_list, prio)
    def _get_slidingw_size(self):
        return 1
    def __str__(self):
        return "PrevANYMatcherARM"
