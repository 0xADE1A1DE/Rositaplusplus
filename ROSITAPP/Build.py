...
from os.path import isfile, join, splitext, splitext, exists
...
EXT_OBJ = ".o"
class Builder():
    def __init__(self,src_config={},cc="",asm="",ld="",cxx="",cxxflags="",extra_flags="",cflags="",lflags="",ldflags="",ldendflags="",cwd=".",objcopy="",buildfile=""):
        self.cc = cc
        self.cxx = cxx
        self.cflags = cflags
        ...
        if isfile(buildjson):
            ...
            self.post_build_run = None
            if 'post_build_run' in A.keys():
                Logger.log_info("post build run script:",A['post_build_run'])
                self.post_build_run = A['post_build_run']
        else:
            raise FileNotFoundError(buildfile)
        
        self.cxxflags = self.cxxflags.replace("$extra_flags", extra_flags)
        self.cflags = self.cflags.replace("$extra_flags", extra_flags)
    ...
    def run_post_build(self):
        Logger.log_info(self.cwd, self.post_build_run)
        if self.post_build_run != None:
            for scrp in self.post_build_run:
                if exists(join(self.cwd, scrp)):
                    self._run_command([join(self.cwd, scrp)])
                else:
                    raise FileNotFoundError("can't find:" + scrp)
