    ...
    def init_nopropvals(self):
        ...
        while i < ns - 1:
            ...
            while j < ns:
                ...
                if ((start_eff_range <= i) and (i <= end_eff_range)) and ((start_eff_range <= j and j <= end_eff_range)):
                    if n.abs(self.tvals[flatidx]) > self.TH: # n.abs(self.tvals[flatidx]) > 4.5:
                        self.tvals_lookup[i].append((i,j,flatidx))
                        self.tvals_lookup[j].append((j,i,flatidx))
