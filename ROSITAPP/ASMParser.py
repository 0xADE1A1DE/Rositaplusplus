...
class DefaultMode:
    ...
    taggedcomment = re.compile(r".*@ edited ({[0-9a-zA-Z, ']+}) hash {([0-9a-z]+)}")
    jumps = []
...
class ASMInst:
    def __init__(self, linewithcomments, line_num, asmfile):
        self.content = asmfile.mode.strip_comments(linewithcomments)
        self.fullline = linewithcomments
        self.line_num = line_num
        self.asmfile = asmfile
    def __str__(self):
        return str(self.line_num) + ":" + self.content 
    def get_last_editmarkerset(self):
        ret = self.asmfile.mode.taggedcomment.match(self.fullline)
        if ret:
            return ret[1]
        return None
    ...
    def get_func_name(self):
        ...
        return None
    def get_state(self):
        return self.asmfile.mode.get_state(self.content)
    def get_op(self):
        ...
        for line in of.readlines():
            ...
            if len(line) > 0:
                if line.startswith("."):
                    ...
                    if label_groups:
                        asmline = ASMInst(linewithcomments, line_num, asmfile)
