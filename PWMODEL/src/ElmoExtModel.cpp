...
typedef struct _PowerModel {
    ...
    double HDPrvOp1CurrOp2[1][5]; // cross op1 ^ op2 HD
    double HDPrvOp2CurrOp1[1][5]; // cross op2 ^ op1 HD
} PowerModel;
...
void load_coeffs(char* coeffsfile, PowerModel* pwModel)
{
    ...
    readcoeffs(pwModel->HDPrvOp1CurrOp2,fpcoeffs, 1);
    readcoeffs(pwModel->HDPrvOp2CurrOp1,fpcoeffs, 1);
    /*for (int i=0;i<5;i++)
    {
        pwModel->HDPrvOp1CurrOp2[0][i] = 0;
        pwModel->HDPrvOp2CurrOp1[0][i] = 0;
    }*/
...
inline uint32_t get_instruction_type(uint32_t inst_id)
{
    return inst_id & 0xff;
}
inline uint32_t get_state_class(uint32_t inst_id)
{
...
inline uint32_t get_bit(uint32_t val, uint8_t bit)
{
    return (val >> bit) & 0x1U;
}
inline uint32_t get_bitflip(uint32_t a, uint32_t b, uint8_t bit)
{
    return ((a ^ b) >> bit) & 0x1U;
}
#define N_PROPS 28
/*
 * Power analysis is divided in to two categories.
 * The individual terms are calculated using termcombs
 * Final power values are calculated using a separate
 * pwanayzer instance.
 *
 **/
void elmoextmodel_t::init(pwmodel_t* pm, const char* execfile)
{
    load_coeffs("../coeffs.txt", &powermodel);
    _totaltraces = pwutil_get_tracecount(execfile);
    
//    pwanalyzer_init(&_power, _totaltraces);
    
    _nsamples = 0;
    _termsamples = nullptr;
    _powersamples = nullptr;
    _data = new uint8_t[1];
    char *dump_traces_file = getenv("PWL_PARAM_DUMP_TRACES_FILE");
    if (dump_traces_file)
    {
//        pwanalyzer_set_param_str(_power, PWL_PARAM_DUMP_TRACES_FILE, dump_traces_file);
    }
#ifdef PWPROPS_ENABLE
    pwprops_init(&_props, 2, N_PROPS, 4000, execfile); 
#endif
}
void elmoextmodel_t::finit(pwmodel_t* pm)
{
    FILE* fmeta=fopen("./output/meta.json","w");
    fprintf(fmeta, "{\n");
    fprintf(fmeta, "    \"analysis\":\"%s\",\n", pwconfig_get_value("ANALYSIS"));
    
    std::vector<char> dumpfilepath(256);
    std::vector<char> absdumpfilepath(256);
    
    const char *outputpath = pwconfig_get_value("OUTPUTDIR");
    
    snprintf(dumpfilepath.data(), 256, "%s/terms-dump.npy", outputpath);
    pwutil_abspath(dumpfilepath.data(), absdumpfilepath.data(), 256);
    
    fprintf(fmeta, "    \"terms_dump\":\"%s\",\n", absdumpfilepath.data());
    fprintf(fmeta, "    \"ntraces\":%d,\n", _totaltraces);
    fprintf(fmeta, "    \"nsamples\":%d\n", _nsamples);
    fprintf(fmeta, "}\n");
    fclose(fmeta);

    pwtermset_dump_output(_ts, "./output/fixedvsrandompropttest.txt");
    pwtermset_dump_output(_tpower, "./output/fixedvsrandomtstatistics.txt");

    //termcombs_finit(_tc);
    pwtermset_finit(_ts);
    pwtermset_finit(_tpower);
    free(_termsamples);
    free(_powersamples);
#ifdef PWPROPS_ENABLE
    pwprops_finit(_props, "./pwprops_output.txt");
#endif
}
FILE *fxdumpfile;
void elmoextmodel_t::on_first_trace_done(pwmodel_t* pm)
{
    _nsamples = pwmodel_get_sample_count(pm);
    _termsamples = (double*) malloc(_nsamples * N_PROPS * sizeof(double));
    _powersamples = (double*) malloc(_nsamples * sizeof(double));
    //termcombs_init(&_tc, N_PROPS, 0, _nsamples, _totaltraces); 
    traceinfo_t termtraceinfo{_nsamples, _totaltraces, N_PROPS, pm->inputdatalength, "terms"};
    traceinfo_t powertraceinfo{_nsamples, _totaltraces, 1, pm->inputdatalength, "power"};
    pwtermset_init(&_ts, &termtraceinfo, "./termsdump.npy");
    pwtermset_init(&_tpower, &powertraceinfo, "./powerdump.npy");
}
void elmoextmodel_t::on_trace_done(pwmodel_t* pm)
{
    ...
    double PrvOp1CurrOp2_data = 0;
    double PrvOp2CurrOp1_data = 0;
    double State_data = 0;
    ...
    while(subsequent->next != NULL){
        ...
        PrvOp1CurrOp2_data = 0;
        PrvOp2CurrOp1_data = 0;
        ...
        if(instructiontype == 5){
            ...
            PrvOp1CurrOp2_data = powermodelptr->HDPrvOp1CurrOp2[0][instructiontype]*hdistance(previous->op1, current->op2);
            PrvOp2CurrOp1_data = powermodelptr->HDPrvOp2CurrOp1[0][instructiontype]*hdistance(previous->op2, current->op1);
...
*/
            ...
            differentialvoltage = powermodelptr->constant[instructiontype] + PrvInstr_data + SubInstr_data + Operand1_data + Operand2_data +
                ...
                State_data + Op1Op2_Overwrite_data + PrvOp1CurrOp2_data + PrvOp2CurrOp1_data;
        ...
        {
            /* reset state according to dominating instructions */
        ...
        else
        {
            ...
            if ( (get_state_class(current->inst_id) != ST_CLS_STR) && (get_state_class(newinst->inst_id) != ST_CLS_STR) && /* if this intr is str it has priority */
            ...
            if ( (get_state_class(current->inst_id) != ST_CLS_EORS) && (get_state_class(newinst->inst_id) != ST_CLS_EORS) && 
        ...
        //LogInfo("%d\n", inst_index);
        _termsamples[0*_nsamples + inst_index] = powermodelptr->constant[instructiontype];
        _termsamples[1*_nsamples + inst_index] = PrvInstr_data;
        _termsamples[2*_nsamples + inst_index] = SubInstr_data;
        _termsamples[3*_nsamples + inst_index] = Operand1_data;
        _termsamples[4*_nsamples + inst_index] = Operand2_data;
        _termsamples[5*_nsamples + inst_index] = BitFlip1_data;
        _termsamples[6*_nsamples + inst_index] = BitFlip2_data;
        _termsamples[7*_nsamples + inst_index] = HWOp1PrvInstr_data;
        _termsamples[8*_nsamples + inst_index] = HWOp2PrvInstr_data;
        _termsamples[9*_nsamples + inst_index] = HDOp1PrvInstr_data;
        _termsamples[10*_nsamples + inst_index] = HDOp2PrvInstr_data;
        _termsamples[11*_nsamples + inst_index] = HWOp1SubInstr_data; 
        _termsamples[12*_nsamples + inst_index] = HWOp2SubInstr_data;
        _termsamples[13*_nsamples + inst_index] = HDOp1SubInstr_data;
        _termsamples[14*_nsamples + inst_index] = HDOp2SubInstr_data; 
        _termsamples[15*_nsamples + inst_index] = Operand1_bitinteractions_data;
        _termsamples[16*_nsamples + inst_index] = Operand2_bitinteractions_data;
        _termsamples[17*_nsamples + inst_index] = BitFlip1_bitinteractions_data;  
        _termsamples[18*_nsamples + inst_index] = BitFlip2_bitinteractions_data; 
        _termsamples[19*_nsamples + inst_index] = state_cum_data[ST_CLS_LDR];
        _termsamples[20*_nsamples + inst_index] = state_cum_data[ST_CLS_STR];
        _termsamples[21*_nsamples + inst_index] = Op1Op2_Overwrite_data; 
        _termsamples[22*_nsamples + inst_index] = NM_Op1_Overwrite_data;
        _termsamples[23*_nsamples + inst_index] = NM_Op2_Overwrite_data;
        _termsamples[24*_nsamples + inst_index] = state_cum_data[ST_CLS_EORS];
        _termsamples[25*_nsamples + inst_index] = state_cum_data[ST_CLS_MOVS];
        _termsamples[26*_nsamples + inst_index] = PrvOp1CurrOp2_data;
        _termsamples[27*_nsamples + inst_index] = PrvOp2CurrOp1_data;
        _powersamples[inst_index] = differentialvoltage;
/*
        ...
        pwanalyzer_trace_update(_analyzers[25], pm->currenttrace, inst_index, differentialvoltage);
*/
#ifdef PWPROPS_ENABLE
    ...
    {
        pwanalyzer_trace_done(_analyzers[i]);
    }*/
    //termcombs_trace_submit(_tc, _termsamples); 
    
    trace_t termstrace;
    traceinfo_t termstraceinfo {_nsamples, _totaltraces, N_PROPS, pm->inputdatalength, "terms"};
    termstrace.init_view(_termsamples, pm->inputdata, &termstraceinfo);
    pwtermset_trace_submit(_ts, &termstrace);
    //pwanalyzer_trace_submit(_power, _powersamples, _nsamples);
    trace_t powertrace;
    traceinfo_t powertraceinfo {_nsamples, _totaltraces, N_PROPS, pm->inputdatalength, "terms"};
    powertrace.init_view(_powersamples, pm->inputdata, &powertraceinfo);
    pwtermset_trace_submit(_tpower, &powertrace);
//    calc_props_fin();
