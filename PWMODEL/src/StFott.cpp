...
void st_fott_t::init(const traceinfo_t* inti, traceinfo_t* outti)
{
    auto p = pimpl.get();
    p->traceinfo = *inti;
    ...
    snprintf(outfilepath.data(), 254, "%s-st-fottest.npy", inti->title.c_str());
    p->fottests = pwfactory_t::get_instance()->new_analysis("first-order-ttest");
    p->aos.init(outfilepath.data(), "numpy");

    p->fottests->init(inti, &p->aos);
}
const trace_t* st_fott_t::process(const trace_t* trace)
{
    auto p = pimpl.get();
    //LogInfo("dd%x\n",trace->_data[0]);
