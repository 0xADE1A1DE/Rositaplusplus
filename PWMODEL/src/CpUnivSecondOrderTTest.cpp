...
struct cp_univ_second_order_ttest_priv_t
{
    std::vector<analysis_output_t> aos;
    std::vector<second_order_ttest_analysis_t> fottests;
    traceinfo_t traceinfo;
    pwtermset_mode_t mode;
};

cp_univ_second_order_ttest_t::~cp_univ_second_order_ttest_t()
{
}
cp_univ_second_order_ttest_t::cp_univ_second_order_ttest_t()
{
...
void cp_univ_second_order_ttest_t::init(const traceinfo_t *traceinfo, pwtermset_mode_t mode)
{
    ...
    for (auto i=0u;i<traceinfo->nterms;i++)
    {
        ...
        snprintf(outsuffix.data(), 254, "sottest-%d.npy", i);
...
void cp_univ_second_order_ttest_t::process(const trace_t* trace)
{
    auto p = pimpl.get();
    if (p->mode == TS_RECORD)
        return;
...
void cp_univ_second_order_ttest_t::finit()
{
    auto p = pimpl.get();
    if (p->mode == TS_RECORD)
    {
