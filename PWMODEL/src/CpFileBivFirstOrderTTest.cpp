...
struct cp_file_biv_first_order_ttest_priv_t
{
...
cp_file_biv_first_order_ttest_t::cp_file_biv_first_order_ttest_t()
{
    pimpl.init();
}
void cp_file_biv_first_order_ttest_t::get_info(cmd_pipeline_info_t& info) 
{ 
    info = cmd_pipeline_info_t{2}; 
}
void cp_file_biv_first_order_ttest_t::init(const traceinfo_t *traceinfo, pwtermset_mode_t mode)
{
...
void cp_file_biv_first_order_ttest_t::init(const char* filename)
{ 
}
void cp_file_biv_first_order_ttest_t::pass_begin(int pass)
{
    pimpl.get()->pass = pass;
}
void cp_file_biv_first_order_ttest_t::process()
{
}
void cp_file_biv_first_order_ttest_t::process(const trace_t* trace)
{
    auto p = pimpl.get();
    ...
    else if (p->pass == 1)
    {
        ...
        auto hh = traceutils_biv_combine(&temptrace, &combinedtr, 
...
void cp_file_biv_first_order_ttest_t::pass_end(int pass)
{
    auto p = pimpl.get();
    ...
    if (p->pass == 0)
    {
        ...
        //snprintf(outfilepath.data(), 254, "%s-biv-fottest.npy", p->traceinfo.title.c_str());
...
void cp_file_biv_first_order_ttest_t::finit()
{
