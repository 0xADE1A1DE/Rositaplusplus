...
cp_triv_first_order_ttest_t::cp_triv_first_order_ttest_t()
{
    pimpl.init();
}
void cp_triv_first_order_ttest_t::init(const traceinfo_t *traceinfo, pwtermset_mode_t mode)
{ 
...
void cp_triv_first_order_ttest_t::process(const trace_t* trace)
{
    auto p = pimpl.get();
...
void cp_triv_first_order_ttest_t::finit()
{
    auto p = pimpl.get();
    ...
    fottesttraceinfo.nsamples = (readtraceinfo.nsamples - 2)*(readtraceinfo.nsamples - 1) *readtraceinfo.nsamples / 6;
    fottesttraceinfo.nterms = 1;
    
    //std::vector<char> outfilepath(255);
    //snprintf(outfilepath.data(), 254, "%s-triv-fottest.npy", readtraceinfo.title.c_str());
    ...
    while (true)
    {
        ...
        auto hh = traceutils_triv_combine(&term0trace, &combinedtr, 
